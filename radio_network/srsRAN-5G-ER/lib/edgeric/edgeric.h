...
private:
    ...
    static bool send_logging_enabled;
...
public:
    static std::atomic<uint32_t> tti_cnt;

    // --- Initialization ---
    static void init();
    static void setTTI(uint32_t tti_count) { tti_cnt.store(tti_count, std::memory_order_relaxed); }

    // --- Logging utility ---
    static void printmyvariables();

    // --- Metric setters ---
    static void set_cqi(uint16_t rnti, float cqi)
    {
        std::lock_guard<std::mutex> lock(metrics_mutex);
        ue_cqis[rnti] = cqi;
    }
    static void set_snr(uint16_t rnti, float snr)
    {
        std::lock_guard<std::mutex> lock(metrics_mutex);
        ue_snrs[rnti] = snr;
    }
    static void set_ul_buffer(uint16_t rnti, uint32_t ul_buffer)
    {
        std::lock_guard<std::mutex> lock(metrics_mutex);
        ue_ul_buffers[rnti] = ul_buffer;
    }
    static void set_dl_buffer(uint16_t rnti, uint32_t dl_buffer)
    {
        std::lock_guard<std::mutex> lock(metrics_mutex);
        ue_dl_buffers[rnti] = dl_buffer;
    }
    static void set_tx_bytes(uint16_t rnti, float tbs); 
    static void set_rx_bytes(uint16_t rnti, float tbs); 
    static void set_dl_tbs(uint16_t rnti, float tbs)
    {
        std::lock_guard<std::mutex> lock(metrics_mutex);
        dl_tbs_ues[rnti] = tbs;
    }

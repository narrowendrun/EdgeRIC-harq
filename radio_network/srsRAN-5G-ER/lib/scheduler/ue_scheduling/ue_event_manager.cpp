...
void ue_event_manager::handle_ue_creation(ue_config_update_event ev)
{
  ...
  common_events.emplace(INVALID_DU_UE_INDEX, [this, u = std::move(u), ev = std::move(ev)]() mutable {
    ...
    for (const auto& ue_ptr : ue_db) {
      if (ue_ptr != nullptr && ue_ptr->crnti != rnti_t::INVALID_RNTI) {
  ...
  for (unsigned i = 0, e = crc_ind.crcs.size(); i != e; ++i) {
    ...
    cell_specific_events[crc_ind.cell_index].emplace(
        ...
        [this, slot_delay, sl_rx = crc_ind.sl_rx, crc = crc_ind.crcs[i]](ue_cell& ue_cc) mutable {
          ...
          //bool acky = crc.tb_crc_success;
          //printf("ul_harq_ack: %d \n",acky);
          ...
          if (tti_cnt_snapshot % 100 == 0) {
            printf("[on_gnb] TTI %u rnti=%u ul_tx_attempt=true ul_harq_ack=%d rx_bytes=%d\n",
                  tti_cnt_snapshot,
                  rnti,
                  ack,

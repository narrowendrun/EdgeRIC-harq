...
uint32_t count_tti = 0;
unsigned this_tti_unused_crbs = 0;
...
alloc_result ue_cell_grid_allocator::allocate_dl_grant(const ue_pdsch_grant& grant)
{
  ...
  for (const ue_pdsch_alloc_param_candidate_searcher::candidate& param_candidate : candidates) {
    ...
    // Calculate the total number of CRBs in the BWP.
    unsigned total_crbs = bwp_dl_cmn.generic_params.crbs.length();
    // Calculate the number of used CRBs.
    unsigned used_crbs_count = used_crbs.count();
    // Calculate the number of unused CRBs.
    unsigned unused_crbs = total_crbs - used_crbs_count;
    

    if (count_tti != edgeric::tti_cnt){
      this_tti_unused_crbs = unused_crbs;
      count_tti = edgeric::tti_cnt;
      }

    // Log the CRB bitmap
    std::ofstream logfile1("scheduling-final.txt", std::ios_base::app); // Open log file in append mode
    if (logfile1.is_open()) {
        logfile1 << "used CRBs before assignment, UE Index: " << static_cast<unsigned int>(ue_cc->rnti()) << ", CRB Bitmap: ";
        for (size_t i = 0; i < used_crbs.size(); ++i) {
            logfile1 << (used_crbs.test(i) ? '1' : '0');
        }
        logfile1 << std::endl;
        logfile1.close();
    }
    float weights_to_log = 0.0;
    ...
    int calc_prbs = mcs_prbs.n_prbs;
    // Try to limit the grant PRBs.
    if (not is_retx) {
      // mcs_prbs.n_prbs = 0.5 * bwp_dl_cmn.generic_params.crbs.length();
      ...
      // }
      // std::ofstream logfile("scheduling-understand.txt", std::ios_base::app); // Open log file in append mode
        // if (logfile.is_open()) {
        //     logfile << ", first compare: " << static_cast<unsigned int>(mcs_prbs.n_prbs);  
        //     // logfile.close();
        // }
      ...
      //  if (logfile.is_open()) {
      //       logfile << ", third compare: " << static_cast<unsigned int>(mcs_prbs.n_prbs);  
      //       // logfile.close();
      //   }

      // mcs_prbs.n_prbs = 0.5 * bwp_dl_cmn.generic_params.crbs.length();
      // mcs_prbs.n_prbs = (static_cast<int>(ue_cc->rnti()) == 17921) ? 
      ...
      // std::ofstream logfile("scheduling-understand.txt", std::ios_base::app); // Open log file in append mode
        // if (logfile.is_open()) {
        //     logfile << ", RNTI: " << static_cast<unsigned int>(ue_cc->rnti());
        //     logfile << ", mcs " << static_cast<unsigned int>(mcs_prbs.mcs.value());
        //     logfile << ", AFTERWARDS: Max PRBs " << static_cast<unsigned int>(grant.max_nof_rbs.value());
        //     logfile << ", allocated PRBs: " << static_cast<unsigned int>(mcs_prbs.n_prbs);
        //     logfile << ", total maybe: " << bwp_dl_cmn.generic_params.crbs.length() << std::endl;   
        //     // logfile.close();
        // }
    }
    ...
    // Log the CRB interval if available
    // std::ofstream logfile("scheduling-understand.txt", std::ios_base::app); // Open log file in append mode
    // if (logfile.is_open()) {
    //     logfile << " CRB Interval: [" << crbs.start() << ", " << crbs.stop() << ")" << std::endl;
    //     logfile.close();
    // }
    ...
    // Convert slot_point to a string for logging
    // std::stringstream slot_stream;
    // slot_stream << "Frame: " << pdsch_alloc.slot.frame << ", Subframe: " << pdsch_alloc.slot.subframe << ", Slot: " << pdsch_alloc.slot.slot;
    // std::string slot_str = slot_stream.str();

    // Convert MCS to an unsigned int for logging
    unsigned int mcs_value = static_cast<unsigned int>(mcs_tbs_info.value().mcs.to_uint());
    // Determine the value to log for opt_weights_recvd
    

    edgeric::set_dl_tbs(static_cast<unsigned int>(ue_cc->rnti()), mcs_tbs_info.value().tbs);
    // Log the final PRBs, MCS, TBS, and UE RNTI after allocation
    std::ofstream logfile("scheduling-final.txt", std::ios_base::app); // Open log file in append mode
    if (logfile.is_open()) {
        logfile << "Final Allocation - UE RNTI: " << static_cast<unsigned int>(u.crnti); // Log UE's RNTI
        ...
        logfile << ", PRBs: " << crbs.length();
        logfile << ", CQI: " << effective_cqi;
        logfile << ", SNR: " << effective_snr;
        logfile << ", TBS: " << mcs_tbs_info.value().tbs << std::endl;
        logfile.close();
    }
